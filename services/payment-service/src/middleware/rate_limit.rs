// Redis-based Rate Limiting untuk Payment Service
...
pub struct RateLimitConfig {
    ...
    pub payment_ops_per_minute: u32,
    ...
    pub fn from_env() -> Self {
        Self {
            ...
            seller_per_minute: env::var("RATE_LIMIT_SELLER_REQUESTS")
                ...
                .parse()
                .unwrap_or(500),
            payment_ops_per_minute: env::var("RATE_LIMIT_SENSITIVE_ENDPOINTS")
                .unwrap_or_else(|_| "30".to_string())
                .parse()
                .unwrap_or(30),
            window_seconds: env::var("RATE_LIMIT_WINDOW_MINUTES")
...
impl RateLimiter {
    ...
    pub fn new(redis_url: &str) -> Result<Self, RateLimitError> {
        ...
        tracing::info!("🔗 Payment Service rate limiter connected to Redis");
    ...
    pub async fn check_rate_limit(
        ...
        let window_key = format!("payment_rate_limit:{}:{}:{}", identifier, role, endpoint);
    ...
    fn get_max_requests(&self, role: &str, endpoint: &str) -> u32 {
        // Payment operations (write) - stricter limit
        let is_payment_op = endpoint.contains("/payments") &&
            (endpoint.contains("POST") || endpoint.contains("PUT") || endpoint.contains("DELETE"));

        // Refund operations - sangat strict
        let is_refund = endpoint.contains("/refund");

        if is_refund {
            self.config.payment_ops_per_minute / 2  
        } else if is_payment_op {
            self.config.payment_ops_per_minute
        } else {
            match role {
